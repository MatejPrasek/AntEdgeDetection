...
void AntColonySystem::DisplayResults(int iter)
{
	...
	for (int i = 0; i < maxWidth; i++)
	{
		for (int j = 0; j < maxHeight; j++)
		{
			const int validNeighbors = GetFoundNeighborsCount(i, j, m_parameters.threshold);
			if(validNeighbors > 6)
				img.at<uchar>(j, i) = 0;
			else
...
int AntColonySystem::GetFoundNeighborsCount(int row, int column, int threshold)
{
	int count = 0;
	if (row - 1 >= 0 && m_parameters.edges[row - 1][column] > threshold)
		count++;
	if (row + 1 < maxWidth && m_parameters.edges[row + 1][column] > threshold)
		count++;

	if (column + 1 < maxHeight && m_parameters.edges[row][column + 1] > threshold)
		count++;
	if (column - 1 >= 0 && m_parameters.edges[row][column - 1] > threshold)
		count++;

	if (column - 1 >= 0 && row - 1 >= 0 && m_parameters.edges[row - 1][column - 1] > threshold)
		count++;

	if (column - 1 >= 0 && row + 1 < maxWidth && m_parameters.edges[row + 1][column - 1] > threshold)
		count++;

	if (column + 1 < maxHeight && row + 1 < maxWidth && m_parameters.edges[row + 1][column + 1] > threshold)
		count++;

	if (column + 1 < maxHeight && row - 1 >= 0 && m_parameters.edges[row - 1][column + 1] > threshold)
		count++;
	return count;
}
