...
int CalculateHeuristic(Mat intensity, int row, int col, int lambda)
{
	int weight = 0;
	...
	if (row - 2 >= 0 && col - 1 >= 0 &&
		row + 2 < maxRow && col + 1 < maxCol)
		weight += abs(int(intensity.at<uchar>(row - 2, col - 1)) - int(intensity.at<uchar>(row + 2, col + 1)));

	if (row + 2 < maxRow && col - 1 >= 0 &&
		row - 2 >= 0 && col + 1 < maxCol)
		weight += abs(int(intensity.at<uchar>(row + 2, col - 1)) - int(intensity.at<uchar>(row - 2, col + 1)));

	if (row - 1 >= 0 && col - 2 >= 0 &&
		row + 1 < maxRow && col + 2 < maxCol)
		weight += abs(int(intensity.at<uchar>(row - 1, col - 2)) - int(intensity.at<uchar>(row + 1, col + 2)));

	if (row + 1 < maxRow && col - 2 >= 0 &&
		row - 1 >= 0 && col + 1 < maxCol)
		weight += abs(int(intensity.at<uchar>(row + 1, col - 2)) - int(intensity.at<uchar>(row - 1, col + 2)));
...
void ParseIntensity(Parameters &parameters)
{
	...
	ofstream intensity;
	intensity.open( "RESULTS/intensity.txt");
	ofstream heuristic;
	heuristic.open("RESULTS/heuristic.txt");
	...
	for (int i = 0; i < grayScaleImage.cols; i++)
	{
		for (int j = 0; j < grayScaleImage.rows; j++)
		{
			...
			parameters.heuristic[i][j] = CalculateHeuristic(grayScaleImage, j, i, parameters.lambda);
